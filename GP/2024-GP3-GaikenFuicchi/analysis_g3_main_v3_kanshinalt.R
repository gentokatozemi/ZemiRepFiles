## 候補者外見年齢実験　分析コード with ０−１を統合した政治関心変数
## 文責：加藤言人
## 作成日：2026/1/13
...
dn$kanshin <- (4 - as.numeric(d$v02)) 
## 逆転して挿入、０−１の範囲にリスケール
table(dn$kanshin, exclude=F) ## 分布を確認
## 0と1を統合
dn$kanshin[dn$kanshin%in%0] <- 1
dn$kanshin <- (dn$kanshin-1)/2
table(dn$kanshin)
# ## ２値化
...
## 0と1を統合
dn$chishiki <- 
  dn$chishiki_1 + dn$chishiki_2 + dn$chishiki_3
dn$chishiki[dn$chishiki%in%0] <- 1
dn$chishiki <- (dn$chishiki-1)/2
table(dn$chishiki)
...
ggplot(plotdata, aes(x=x, y=y)) +
  ...
  theme_bw()
### 保存
ggsave("bunpu_kanshin_kanshinalt.png", width=6, height=4)
...
### まずはジェネリックな表を見る
screenreg(list(m4_m2b,m4_m3b), 
          include.ci=FALSE, digits=4,
          stars = c(0.001,0.01,0.05,0.1), symbol="+",
          custom.header = list("DV:seisakusenko" = 1,
                               "DV:kioku" = 2),
          custom.model.names = c("H4 (H2)",
                                 "H4 (H3)")) 
...
screenreg(list(m4_m2b,m4_m3b), 
          ...
          stars = c(0.001,0.01,0.05,0.1), symbol="+",
          custom.header = list("DV:政策選好" = 1,
                               "DV:公約記憶" = 2),
          custom.model.names = c("H4 (H2)","H4 (H3)"),
          custom.coef.map = vnmap) 

## LaTeXに出力
texreg(list(m4_m2b,m4_m3b), 
       include.ci=FALSE, digits=4,
       stars = c(0.001,0.01,0.05,0.1), symbol="+",
       custom.header = list("DV:政策選好" = 1,
                            "DV:公約記憶" = 2),
       ...
       caption = "仮説4の回帰表（値０と１を統合した政治関心を使用）", 
       caption.above = TRUE,
       booktabs = TRUE, dcolumn = TRUE, use.packages = FALSE,
       fontsize = "scriptsize", label = "tab_m4_kanshinalt", float.pos = "ht!",
       file = "m4tab_kanshinalt.tex") 
...
ggsave("predictionplot_h4_h2_kanshin_kanshinalt.pdf",width=7.5,height=5,
       device = cairo_pdf)
...
ggsave("predictionplot_h4_h3_kanshin_kanshinalt.pdf",
