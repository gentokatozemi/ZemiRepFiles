## 候補者外見年齢実験　分析コード with 正答０−１問を統合した政治知識変数
## 文責：加藤言人
## 作成日：2026/1/13
...
ggplot(plotdata, aes(x=x, y=y)) +
  ...
  labs(x = "政治知識",
       y="回答割合") 
### 保存
ggsave("bunpu_chishiki_chishikialt.png", width=6, height=4)
...
texreg(list(m4_m2b,m4_m3b), 
       ...
       caption = "仮説4の回帰表（正答０−１問を統合した政治知識を使用）", 
       caption.above = TRUE,
       booktabs = TRUE, dcolumn = TRUE, use.packages = FALSE,
       fontsize = "scriptsize", label = "tab_m4_chishikialt", float.pos = "ht!",
       file = "m4tab_chishikialt.tex") 
...
ggsave("predictionplot_h4_h2_chishiki_chishikialt.pdf",width=7.5,height=5,
       device = cairo_pdf)
...
ggsave("predictionplot_h4_h3_chishiki_chishikialt.pdf",
