...
## Exclude three respondents with missing JSDF trust/ideology
dim(dn)
dn <- na.omit(dn)
dim(dn)
...
## Trust in the JSDF and the Prime Minister

### Data for plotting
plotdata <- 
  rbind(data.frame(x = names(table(dn$trust_jsdf)), 
                   y = as.numeric(prop.table(table(dn$trust_jsdf))),
                   label="JSDF"),
        data.frame(x = names(table(dn$trust_pm)), 
                   y = as.numeric(prop.table(table(dn$trust_pm))),
...
ggplot(plotdata, aes(x=x, y=y)) + 
  ...
  facet_grid(.~label) + 
  labs(x = "The level of trust\n(1=don't trust at all; 7=trust very much)", 
       y="Proportion") +
  theme_bw()
### Save
ggsave("./out/descr_trust.pdf", width=6, height=4)

## Ideology

### Data for plotting
plotdata <- data.frame(x = names(table(dn$ideology)), 
                       y = as.numeric(prop.table(table(dn$ideology))))
plotdata$x <- factor(plotdata$x, levels=unique(plotdata$x))
plotdata 
### Figure A.2
ggplot(plotdata, aes(x=x, y=y)) + 
  geom_bar(stat = "identity") + 
  labs(x = "Ideology\n(0=Strong liberal (left-wing); 10=Strong conservative (right-wing))", 
       y="Proportion") +
  theme_bw()
### 保存
ggsave("./out/descr_ideology.pdf", width=6, height=4)
...
## Baseline Models
m00 <- lm_robust(trust_jsdf ~ failedcontrol + casualties, data=dn) 
summary(m00)
...
## Interacted Models
m00x <- lm_robust(trust_jsdf ~ failedcontrol*ideology + casualties, data=dn) 
summary(m00x)
...
texreg(list(m00,m00x,m01,m01x,m02,m02x),
       ...
       table = FALSE, file = "./out/tab_all.tex") 
tmp <- readLines("./out/tab_all.tex")
writeLines(gsub("{dagger}","{\\dagger}",tmp, fixed=TRUE),
           "./out/tab_all.tex")

## PM as an Outcome
m03 <- lm_robust(trust_pm ~ failedcontrol + casualties, data=dn) 
summary(m03)
...
## Interacted Models
m03x <- lm_robust(trust_pm ~ failedcontrol*ideology + casualties, data=dn) 
summary(m00x)
...
texreg(list(m03,m03x,m031,m031x,m032,m032x),
       ...
       custom.note = "%stars. Robust standard errors in parentheses.",       
       booktabs = TRUE, dcolumn = TRUE, use.packages = FALSE,
       table = FALSE, file = "./out/tab_all_pm.tex") 
tmp <- readLines("./out/tab_all_pm.tex")
writeLines(gsub("{dagger}","{\\dagger}",tmp, fixed=TRUE),
           "./out/tab_all_pm.tex")
...
## Baseline Treatment Effects (for reference)
cfdt <- as.data.frame(
...
ggsave("./out/fig_maineff.pdf", width=6, height=3)
...
ggsave("./out/fig_pred.pdf", width=7, height=3)
...
ggsave("./out/fig_maineff_interacted.pdf", width=7, height=3)
...
ggplot(na.omit(printeracted), aes(x=labelledx, y=pr)) + 
  ...
  theme(legend.position="bottom",
        plot.subtitle = element_text(hjust=0.5))
ggsave("./out/fig_pred_interacted.pdf", width=7, height=3)
...
ggplot(cfdt, aes(x=est, y=mname, xmin=lo95, xmax=up95)) + 
  ...
  theme_bw() + 
  theme(axis.text.y = element_text(size=12))
ggsave("./out/fig_maineff_pm.pdf", width=6, height=3)
...
ggplot(prbase, aes(x=labelledx, y=pr)) + 
  ...
  theme_bw() + 
  theme(plot.subtitle = element_text(hjust=0.5))
ggsave("./out/fig_pred_pm.pdf", width=7, height=3)
...
ggplot(cfdt, aes(y=est, x=mod, ymin=lo95, ymax=up95)) + 
  ...
  theme(panel.grid.minor = element_blank(),
        panel.grid.major.y = element_blank(),
        plot.subtitle = element_text(hjust=0.5))
ggsave("./out/fig_maineff_interacted_pm.pdf", width=7, height=3)
...
ggplot(na.omit(printeracted), aes(x=labelledx, y=pr)) + 
  ...
  theme(legend.position="bottom",
        plot.subtitle = element_text(hjust=0.5))
ggsave("./out/fig_pred_interacted_pm.pdf", width=7, height=3)
