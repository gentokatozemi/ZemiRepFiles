...
### pretreatment
table(d$v07, exclude=F) ## distribution
dn$trust_jsdf_pre <- as.numeric(d$v07)　## insert
table(dn$trust_jsdf_pre, exclude=F) ## distribution
...
### pretreatment
table(d$v04, exclude=F) ## distribution
dn$trust_pm_pre <- as.numeric(d$v04)　## insert
table(dn$trust_pm_pre, exclude=F) ## distribution
...
vnmap <- list("(Intercept)" = "(Intercept)",
              ...
              "trust_jsdf_pre" = "Trust in the JSDF (pretreatment)",
              "failedcontrol:trust_jsdf_pre" = "Failure * pre-JSDF trust",
              "trust_pm_pre" = "Trust in the PM (pretreatment)",
              "failedcontrol:trust_pm_pre" = "Failure * pre-PM trust")

## Baseline Models
m00 <- lm_robust(trust_jsdf ~ failedcontrol + casualties + 
                   trust_jsdf_pre, data=dn) 
summary(m00)
...
texreg(list(m00,m00x,m01,m01x,m02,m02x),
       ...
       table = FALSE, file = "./out/withpredv_nosatisficer_tab_all.tex") 
tmp <- readLines("./out/withpredv_nosatisficer_tab_all.tex")
writeLines(gsub("{dagger}","{\\dagger}",tmp, fixed=TRUE),
           "./out/withpredv_nosatisficer_tab_all.tex")

## PM as an Outcome
m03 <- lm_robust(trust_pm ~ failedcontrol + casualties + 
                   trust_pm_pre, data=dn) 
summary(m03)
...
texreg(list(m03,m03x,m031,m031x,m032,m032x),
       ...
       custom.note = "%stars. Robust standard errors in parentheses.",       
       booktabs = TRUE, dcolumn = TRUE, use.packages = FALSE,
       table = FALSE, file = "./out/withpredv_nosatisficer_tab_all_pm.tex") 
tmp <- readLines("./out/withpredv_nosatisficer_tab_all_pm.tex")
writeLines(gsub("{dagger}","{\\dagger}",tmp, fixed=TRUE),
           "./out/withpredv_nosatisficer_tab_all_pm.tex")
...
ggsave("./out/withpredv_nosatisficer_fig_maineff.pdf", width=6, height=3)
...
ggsave("./out/withpredv_nosatisficer_fig_pred.pdf", width=7, height=3)
...
ggsave("./out/withpredv_nosatisficer_fig_maineff_interacted.pdf", width=7, height=3)
...
ggplot(na.omit(printeracted), aes(x=labelledx, y=pr)) + 
  ...
  theme(legend.position="bottom",
        plot.subtitle = element_text(hjust=0.5))
ggsave("./out/withpredv_nosatisficer_fig_pred_interacted.pdf", width=7, height=3)
...
ggplot(cfdt, aes(x=est, y=mname, xmin=lo95, xmax=up95)) + 
  ...
  theme_bw() + 
  theme(axis.text.y = element_text(size=12))
ggsave("./out/withpredv_nosatisficer_fig_maineff_pm.pdf", width=6, height=3)
...
ggplot(prbase, aes(x=labelledx, y=pr)) + 
  ...
  theme_bw() + 
  theme(plot.subtitle = element_text(hjust=0.5))
ggsave("./out/withpredv_nosatisficer_fig_pred_pm.pdf", width=7, height=3)
...
ggplot(cfdt, aes(y=est, x=mod, ymin=lo95, ymax=up95)) + 
  ...
  theme(panel.grid.minor = element_blank(),
        panel.grid.major.y = element_blank(),
        plot.subtitle = element_text(hjust=0.5))
ggsave("./out/withpredv_nosatisficer_fig_maineff_interacted_pm.pdf", width=7, height=3)
...
ggplot(na.omit(printeracted), aes(x=labelledx, y=pr)) + 
  ...
  theme(legend.position="bottom",
        plot.subtitle = element_text(hjust=0.5))
ggsave("./out/withpredv_nosatisficer_fig_pred_interacted_pm.pdf", width=7, height=3)
