...
texreg(list(m00,m00x,m01,m01x,m02,m02x),
       ...
       table = FALSE, file = "./out/withpredv_tab_all.tex") 
tmp <- readLines("./out/withpredv_tab_all.tex")
writeLines(gsub("{dagger}","{\\dagger}",tmp, fixed=TRUE),
           "./out/withpredv_tab_all.tex")
...
texreg(list(m03,m03x,m031,m031x,m032,m032x),
       ...
       custom.note = "%stars. Robust standard errors in parentheses.",       
       booktabs = TRUE, dcolumn = TRUE, use.packages = FALSE,
       table = FALSE, file = "./out/withpredv_tab_all_pm.tex") 
tmp <- readLines("./out/withpredv_tab_all_pm.tex")
writeLines(gsub("{dagger}","{\\dagger}",tmp, fixed=TRUE),
           "./out/withpredv_tab_all_pm.tex")
...
ggsave("./out/withpredv_fig_maineff.pdf", width=6, height=3)
...
ggsave("./out/withpredv_fig_pred.pdf", width=7, height=3)
...
ggsave("./out/withpredv_fig_maineff_interacted.pdf", width=7, height=3)
...
ggplot(na.omit(printeracted), aes(x=labelledx, y=pr)) + 
  ...
  theme(legend.position="bottom",
        plot.subtitle = element_text(hjust=0.5))
ggsave("./out/withpredv_fig_pred_interacted.pdf", width=7, height=3)
...
ggplot(cfdt, aes(x=est, y=mname, xmin=lo95, xmax=up95)) + 
  ...
  theme_bw() + 
  theme(axis.text.y = element_text(size=12))
ggsave("./out/withpredv_fig_maineff_pm.pdf", width=6, height=3)
...
ggplot(prbase, aes(x=labelledx, y=pr)) + 
  ...
  theme_bw() + 
  theme(plot.subtitle = element_text(hjust=0.5))
ggsave("./out/withpredv_fig_pred_pm.pdf", width=7, height=3)
...
ggplot(cfdt, aes(y=est, x=mod, ymin=lo95, ymax=up95)) + 
  ...
  theme(panel.grid.minor = element_blank(),
        panel.grid.major.y = element_blank(),
        plot.subtitle = element_text(hjust=0.5))
ggsave("./out/withpredv_fig_maineff_interacted_pm.pdf", width=7, height=3)
...
ggplot(na.omit(printeracted), aes(x=labelledx, y=pr)) + 
  ...
  theme(legend.position="bottom",
        plot.subtitle = element_text(hjust=0.5))
ggsave("./out/withpredv_fig_pred_interacted_pm.pdf", width=7, height=3)
